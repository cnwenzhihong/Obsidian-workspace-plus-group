...
const DEFAULT_SETTINGS = {
    ...
    workspaceChildren: {},
    collapsedWorkspaces: {},
    modalCollapseMode: "inherit",
    modalCollapsedWorkspaces: {},
};
...
const STRINGS = {
    ...
    "modal-collapse-mode-all-expanded": { en: "Always expand all", zh: "始终全部展开" },
...
function getLang() {
    ...
    const ml = window.moment && window.moment.locale ? window.moment.locale() : "";
    _lang = (ml && ml.toLowerCase().startsWith("zh")) ? "zh" : "en";
    return _lang;
}
function t(key) {
    return (STRINGS[key] && STRINGS[key][getLang()]) || (STRINGS[key] && STRINGS[key].en) || key;
}
...
class WorkspacesPlusSettingsTab extends obsidian.PluginSettingTab {
    ...
    addSection(containerEl, sectionId, sectionName) {
        const collapsed = this.plugin.settings.collapsedSections[sectionId];
        new obsidian.Setting(containerEl)
            ...
            const iconContainer = createSpan({ cls: "settings-collapse-indicator" });
            obsidian.setIcon(iconContainer, "right-triangle");
            setting.nameEl.appendChild(iconContainer);
            setting.settingEl.addEventListener("click", () => {
                ...
                this.plugin.settings.collapsedSections[sectionId] = setting.settingEl.hasClass("is-collapsed");
                this.plugin.saveData(this.plugin.settings);
            });
        });
        return containerEl.createDiv({ cls: "settings-container" });
    }
    display() {
        ...
        if (!this.plugin.utils.isNativePluginEnabled) {
            containerEl.createEl("h2", {
                text: t("enable-workspaces-plugin"),
            });
            return;
        }
        new obsidian.Setting(containerEl)
            .setName(t("auto-save-layout"))
            .setDesc(t("auto-save-layout-desc"))
            .addToggle(toggle => toggle.setValue(this.plugin.settings.saveOnChange).onChange(value => {
            this.plugin.settings.saveOnChange = value;
            this.plugin.saveData(this.plugin.settings);
        }));
        const displayEl = this.addSection(containerEl, "display", t("display"));
        new obsidian.Setting(displayEl)
            .setName(t("show-delete-confirmation"))
        ...
        new obsidian.Setting(advancedEl)
            ...
            e.appendText(t("workspace-modes")),
                e.createSpan({ cls: "flair mod-pop", text: "beta" });
        }))
            .setDesc(t("workspace-modes-desc"))
            .then(setting => {
        ...
        const modeSubEl = advancedEl.createDiv({ cls: "requires-workspace-modes mode-sub-settings" });
        new obsidian.Setting(modeSubEl)
            .setName(t("show-mode-ribbon"))
        ...
        new obsidian.Setting(modeSubEl)
            .setName(t("system-dark-mode"))
            .setDesc(t("system-dark-mode-desc"))
        ...
        new obsidian.Setting(modeSubEl)
            .setName(t("auto-reload-live-preview"))
            .setDesc(t("auto-reload-live-preview-desc"))
    ...
    renderHierarchy(treeContainer) {
        treeContainer.empty();
        const allNames = Object.keys(this.plugin.workspacePlugin.workspaces)
            .filter(w => !/^mode:/i.test(w))
            .sort();
        const collapsed = this.plugin.settings.collapsedWorkspaces;
        const handled = new Set();
        // Root-level: __root__ items first (preserving manual order), then orphans sorted
        const rootOrdered = [];
        const rootSet = new Set();
        for (const name of (this.plugin.settings.workspaceChildren["__root__"] || [])) {
            if (allNames.includes(name) && !rootSet.has(name)) {
                rootOrdered.push(name);
                rootSet.add(name);
            }
        }
        for (const name of allNames) {
            if (!rootSet.has(name) && !this.plugin.getWorkspaceParent(name)) {
                rootOrdered.push(name);
                rootSet.add(name);
            }
        }
        const renderLevel = (parentName, depth) => {
            const children = this.plugin.getWorkspaceChildren(parentName);
            for (const name of children) {
                if (!allNames.includes(name)) continue;
                if (handled.has(name)) continue;
                handled.add(name);
                this.renderHierarchyRow(treeContainer, name, depth);
                if (!collapsed[name]) {
                    renderLevel(name, depth + 1);
                }
            }
        };
        for (const name of rootOrdered) {
            if (handled.has(name)) continue;
            handled.add(name);
            this.renderHierarchyRow(treeContainer, name, 0);
            if (!collapsed[name]) {
                renderLevel(name, 1);
            }
        }
    }
    renderHierarchyRow(treeContainer, name, depth = 0) {
        const row = treeContainer.createDiv({ cls: "hierarchy-row" });
        row.setAttribute("draggable", "true");
        row.dataset.workspaceName = name;
        row.style.paddingLeft = `${depth * 24 + 8}px`;
        const children = this.plugin.getWorkspaceChildren(name);
        const collapsed = this.plugin.settings.collapsedWorkspaces[name];
        // Collapse toggle
        if (children.length > 0) {
            const collapseBtn = row.createSpan({ cls: "hierarchy-collapse" });
            collapseBtn.innerHTML = collapsed
                ? `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="12" height="12"><path fill="currentColor" d="M5 2l6 6-6 6V2z"/></svg>`
                : `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="12" height="12"><path fill="currentColor" d="M2 5l6 6 6-6H2z"/></svg>`;
            collapseBtn.setAttribute("aria-label", collapsed ? t("expand-workspace") : t("collapse-workspace"));
            collapseBtn.addEventListener("click", (e) => {
                e.stopPropagation();
                this.plugin.settings.collapsedWorkspaces[name] = !collapsed;
                this.plugin.saveData(this.plugin.settings);
                const treeEl = row.parentElement;
                this.renderHierarchy(treeEl);
            });
        }
        else {
            row.createSpan({ cls: "hierarchy-collapse hierarchy-collapse-placeholder" });
        }
        // Drag grip
        const grip = row.createSpan({ cls: "hierarchy-grip" });
        grip.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="12" height="12"><circle cx="4" cy="4" r="1.5" fill="currentColor"/><circle cx="4" cy="8" r="1.5" fill="currentColor"/><circle cx="4" cy="12" r="1.5" fill="currentColor"/><circle cx="10" cy="4" r="1.5" fill="currentColor"/><circle cx="10" cy="8" r="1.5" fill="currentColor"/><circle cx="10" cy="12" r="1.5" fill="currentColor"/></svg>`;
        // Name
        const nameEl = row.createSpan({ cls: "hierarchy-name", text: name });
        ...
        row.addEventListener("dragover", (e) => {
            ...
            if (!this._dragName || this._dragName === name) return;
...
class WorkspacesPlusPluginWorkspaceModal extends obsidian.FuzzySuggestModal {
    constructor(plugin, settings, hotkey = false) {
        ...
        this.useSelectedItem = function (evt) {
            ...
            const inputEl = this.modalEl.querySelector(".prompt-input");
            let workspaceName = inputEl && inputEl.value ? inputEl.value : (this.chooser.values ? this.chooser.values[this.chooser.selectedItem].item : "");
            ...
            if (workspaceName && !this.getItems().includes(workspaceName)) {
                this.saveAndStay();
                return false;
            }
        ...
        this.onRenameClick = function (evt, el) {
            ...
            if (el.contentEditable === "true") {
                ...
                // Confirm button inside the editing field
                const confirmBtn = createSpan({ cls: "rename-confirm-btn" });
                confirmBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18"><path fill="none" d="M0 0h24v24H0z"/><path fill="currentColor" d="M10 15.172l9.192-9.193 1.415 1.414L10 18l-6.364-6.364 1.414-1.414z"/></svg>`;
        ...
        this.setPlaceholder(t("type-workspace-name"));
        ...
        this.modalEl.classList.add("workspace-plus-group-modal");
    ...
    onNoSuggestion() {
        ...
        this.chooser.addMessage(t("press-enter-to-save"));
        const el = this.chooser.containerEl.querySelector(".suggestion-empty");
        el.createEl("button", {
            cls: "list-item-part",
            text: t("save-as-new-workspace"),
    ...
    open() {
        ...
        if (!this.invokedViaHotkey) {
            this.popper = createPopper(document.body.querySelector(".plugin-workspace-plus-group"), this.modalEl, {
                placement: "top-start",
    ...
    saveAndStay() {
        const inputEl = this.modalEl.querySelector(".prompt-input");
        let workspaceName = inputEl && inputEl.value ? inputEl.value : this.chooser.values[this.chooser.selectedItem].item;
        this.workspacePlugin.saveWorkspace(workspaceName);
        this.chooser.chooser.updateSuggestions();
        if (!/^mode:/i.test(workspaceName))
            this.setWorkspace(workspaceName);
        new obsidian.Notice(t("saved-workspace") + workspaceName);
        this.close();
    }
    saveAndSwitch() {
        this.workspacePlugin.saveWorkspace(this.activeWorkspace);
        this.plugin.registerWorkspaceHotkeys();
        new obsidian.Notice(t("saved-workspace") + this.activeWorkspace);
    }
    ...
    renderSuggestion(item, el) {
        ...
        const depth = this.plugin.getWorkspaceDepth(workspaceName);
        el.style.paddingLeft = `${20 + depth * 16}px`;
        const hasChildren = this.plugin.getWorkspaceChildren(workspaceName).length > 0;
        const collapsed = this.plugin.getModalCollapseState();
        const isCollapsed = collapsed[workspaceName];
        const resultEl = document.body.querySelector("div.workspace-plus-group-modal div.prompt-results");
        ...
        // Collapse button: always present before text for alignment,
        // hidden (visibility) when no children to reserve the space
        let collapseBtn = el.querySelector(".hierarchy-modal-collapse");
        if (!collapseBtn) {
            collapseBtn = createSpan({ cls: "hierarchy-modal-collapse" });
            el.prepend(collapseBtn);
            collapseBtn.addEventListener("click", (e) => {
                e.stopPropagation();
                const targetStore = this.plugin.settings.modalCollapseMode === "inherit"
                    ? this.plugin.settings.collapsedWorkspaces
                    : this.plugin.settings.modalCollapsedWorkspaces;
                targetStore[el.dataset.workspaceName] = !targetStore[el.dataset.workspaceName];
                this.plugin.saveData(this.plugin.settings);
                const currentItem = this.chooser.values ? this.chooser.values[this.chooser.selectedItem] : null;
                this.chooser.chooser.updateSuggestions();
                if (currentItem) {
                    const newValues = this.chooser.values;
                    if (newValues) {
                        const newIdx = newValues.findIndex(v => v.item === currentItem.item);
                        if (newIdx >= 0) this.chooser.setSelectedItem(newIdx, null);
                    }
                }
            });
        }
        collapseBtn.removeClass("has-children");
        if (hasChildren) {
            collapseBtn.addClass("has-children");
            collapseBtn.innerHTML = isCollapsed
                ? `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path fill="currentColor" d="M5 2l6 6-6 6V2z"/></svg>`
    ...
    doDelete(workspaceName) {
        let currentSelection = this.chooser.selectedItem;
        const scrollTop = this.modalEl.querySelector(".prompt-results")?.scrollTop || 0;
        ...
        requestAnimationFrame(() => {
            const el = this.modalEl.querySelector(".prompt-results");
    ...
    getItems() {
        ...
        const result = [];
        const processWorkspace = (name) => {
            if (!allNames.includes(name) || visited.has(name)) return;
            visited.add(name);
            result.push(name);
            if (!collapsed[name]) {
                const children = this.plugin.getWorkspaceChildren(name);
                for (const child of children) {
                    processWorkspace(child);
                }
            }
        };
        const roots = this.plugin.settings.workspaceChildren["__root__"] || [];
...
const SETTINGS_ATTR = "workspace-plus-group:settings-v1";
class WorkspacesPlusPluginModeModal extends obsidian.FuzzySuggestModal {
    constructor(plugin, settings, hotkey = false) {
        ...
        this.invokedViaHotkey = hotkey;
        this.workspacePlugin = this.app.internalPlugins.getPluginById("workspaces").instance;
        this.setPlaceholder(t("type-mode-name"));
        // temporary styling to force a transparent modal background to address certain themes
        // that apply a background to the modal container instead of the modal-bg
        this.bgEl.parentElement.setAttribute("style", "background-color: transparent !important");
        this.modalEl.classList.add("workspace-plus-group-mode-modal");
        // handle custom modal positioning when invoked via the status bar
        if (!this.invokedViaHotkey) {
            this.bgEl.setAttribute("style", "background-color: transparent");
            this.modalEl.classList.add("quick-switch");
    ...
    open() {
        ...
        document.body.appendChild(this.containerEl);
        if (!this.invokedViaHotkey) {
            this.popper = createPopper(document.body.querySelector(".plugin-workspace-plus-group.mode-switcher"), this.modalEl, {
                placement: "top-start",
                modifiers: [{ name: "offset", options: { offset: [0, 10] } }],
            });
        }
    ...
    saveAndStay() {
        ...
        this.chooser.chooser.updateSuggestions();
        new obsidian.Notice(t("saved-mode") + ": " + workspaceName);
    }
    deleteWorkspace(workspaceName = null) {
        if (!workspaceName) {
            let currentSelection = this.chooser.selectedItem;
        ...
        if (this.settings.showDeletePrompt) {
            createConfirmationDialog(this.app, {
                ...
                text: t("mode-delete-confirm-text").replace("{}", workspaceName),
                title: t("mode-delete-confirm-title"),
            });
        }
        else {
            this.doDelete("Mode: " + workspaceName);
    ...
    renderSuggestion(item, el) {
        super.renderSuggestion(item, el);
        const resultEl = document.body.querySelector("div.workspace-plus-group-mode-modal div.prompt-results");
    ...
    addRenameButton(wrapperEl, el) {
        const renameIcon = wrapperEl.createDiv("rename-workspace");
        renameIcon.setAttribute("aria-label", t("rename-mode"));
        renameIcon.setAttribute("aria-label-position", "top");
        renameIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"><path fill="none" d="M0 0h24v24H0z"/><path d="M12.9 6.858l4.242 4.243L7.242 21H3v-4.243l9.9-9.9zm1.414-1.414l2.121-2.122a1 1 0 0 1 1.414 0l2.829 2.829a1 1 0 0 1 0 1.414l-2.122 2.121-4.242-4.242z"/></svg>`;
        renameIcon.addEventListener("click", event => this.onRenameClick(event, el));
    }
    addDeleteButton(wrapperEl) {
        const deleteIcon = wrapperEl.createDiv("delete-workspace");
        deleteIcon.setAttribute("aria-label", t("delete-mode"));
        deleteIcon.setAttribute("aria-label-position", "top");
        deleteIcon.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"><path fill="none" d="M0 0h24v24H0z"/><path d="M7 4V2h10v2h5v2h-2v15a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6H2V4h5zM6 6v14h12V6H6zm3 3h2v8H9V9zm4 0h2v8h-2V9z"/></svg>`;
...
class Utils {
    constructor(plugin) {
        this.SETTINGS_ATTR = "workspace-plus-group:settings-v1";
        this.plugin = plugin;
        this.app = plugin.app;
        this.workspacePlugin = this.app.internalPlugins.getPluginById("workspaces").instance;
    }
...
class WorkspacesPlus extends obsidian.Plugin {
    constructor() {
        ...
        this.setWorkspaceName = obsidian.debounce(() => {
            ...
            if (!this.isNativePluginEnabled) {
                (_a = this.changeWorkspaceButton) === null || _a === void 0 ? void 0 : _a.setText(t("error-workspaces-disabled"));
            }
            else {
        ...
        this.onWorkspaceDelete = (workspaceName) => {
            this.setWorkspaceName();
            this.cleanupHierarchy(workspaceName);
            this.saveData(this.settings);
    ...
    getWorkspaceChildren(name) {
        return this.settings.workspaceChildren[name] || [];
    }
    getWorkspaceDepth(name) {
        let depth = 0;
        let current = this.getWorkspaceParent(name);
        while (current) {
            depth++;
            current = this.getWorkspaceParent(current);
        }
        return depth;
    }
    getOrderedWorkspaces() {
        const allNames = Object.keys(this.workspacePlugin.workspaces).filter(w => !/^mode:/i.test(w)).sort();
        const visited = new Set();
        const result = [];
        const traverse = (name) => {
            if (visited.has(name))
            ...
            if (this.workspacePlugin.workspaces[name])
                result.push(name);
            for (const child of this.getWorkspaceChildren(name)) {
                traverse(child);
            }
        };
        // Root-level: __root__ items first (manual order), then orphans sorted
        const rootOrdered = [];
        const rootSet = new Set();
        for (const name of (this.settings.workspaceChildren["__root__"] || [])) {
            if (allNames.includes(name) && !rootSet.has(name)) {
                rootOrdered.push(name);
                rootSet.add(name);
            }
        ...
        for (const name of allNames) {
            if (!rootSet.has(name) && !this.getWorkspaceParent(name)) {
                rootOrdered.push(name);
                rootSet.add(name);
            }
        }
        for (const root of rootOrdered) {
            traverse(root);
        }
        // Any remaining (defensive)
        for (const name of allNames) {
            if (!visited.has(name))
                result.push(name);
        }
    ...
    cleanupHierarchy(workspaceName) {
        if (this.settings.workspaceChildren["__root__"]) {
            this.settings.workspaceChildren["__root__"] =
                this.settings.workspaceChildren["__root__"].filter(w => w !== workspaceName);
        }
        for (const [parent, children] of Object.entries(this.settings.workspaceChildren)) {
            if (parent !== "__root__") {
                this.settings.workspaceChildren[parent] = children.filter(w => w !== workspaceName);
            }
        }
        delete this.settings.workspaceChildren[workspaceName];
        delete this.settings.collapsedWorkspaces[workspaceName];
        delete this.settings.modalCollapsedWorkspaces[workspaceName];
    }
    renameInHierarchy(oldName, newName) {
        const rootList = this.settings.workspaceChildren["__root__"];
        if (rootList) {
            const idx = rootList.indexOf(oldName);
            if (idx !== -1)
                rootList[idx] = newName;
        }
        for (const [parent, children] of Object.entries(this.settings.workspaceChildren)) {
            const idx = children.indexOf(oldName);
            if (idx !== -1)
                this.settings.workspaceChildren[parent][idx] = newName;
        }
        if (this.settings.workspaceChildren[oldName]) {
            this.settings.workspaceChildren[newName] = this.settings.workspaceChildren[oldName];
            delete this.settings.workspaceChildren[oldName];
        }
        if (this.settings.collapsedWorkspaces[oldName] !== undefined) {
            this.settings.collapsedWorkspaces[newName] = this.settings.collapsedWorkspaces[oldName];
            delete this.settings.collapsedWorkspaces[oldName];
        }
        if (this.settings.modalCollapsedWorkspaces[oldName] !== undefined) {
            this.settings.modalCollapsedWorkspaces[newName] = this.settings.modalCollapsedWorkspaces[oldName];
            delete this.settings.modalCollapsedWorkspaces[oldName];
        }
    }
    getModalCollapseState() {
        const allNames = Object.keys(this.workspacePlugin.workspaces).filter(w => !/^mode:/i.test(w));
        switch (this.settings.modalCollapseMode) {
            case "all-expanded":
                return Object.assign({}, this.settings.modalCollapsedWorkspaces);
            case "all-collapsed":
                const allState = {};
                for (const name of allNames)
                    allState[name] = true;
                for (const [name, val] of Object.entries(this.settings.modalCollapsedWorkspaces))
                    allState[name] = val;
                return allState;
            case "independent":
                return Object.assign({}, this.settings.modalCollapsedWorkspaces);
    ...
    updateRibbonDisplay() {
        if (this.settings.workspaceSwitcherRibbon) {
            if (!this.ribbonIconWorkspaces) {
                this.ribbonIconWorkspaces = this.addRibbonIcon("pane-layout", t("manage-workspaces"), () => __awaiter(this, void 0, void 0, function* () { return new WorkspacesPlusPluginWorkspaceModal(this, this.settings, true).open(); }));
            }
            this.ribbonIconWorkspaces.style.display = "";
        }
        else if (this.ribbonIconWorkspaces) {
            this.ribbonIconWorkspaces.style.display = "none";
        }
    }
    toggleModeRibbonButton() {
        if (this.settings.modeSwitcherRibbon) {
            if (!this.ribbonIconMode) {
                this.ribbonIconMode = this.addRibbonIcon("gear", t("manage-modes"), () => __awaiter(this, void 0, void 0, function* () { return new WorkspacesPlusPluginModeModal(this, this.settings, true).open(); }));
            }
            this.ribbonIconMode.style.display = "";
        }
        else if (this.ribbonIconMode) {
            this.ribbonIconMode.style.display = "none";
        }
    }
    enableModesFeature() {
        if (this.settings.workspaceSettings) {
            ...
            this.addCommand({
                id: "open-workspace-plus-group-modes",
                name: t("open-workspace-plus-group-modes"),
    ...
    disableModesFeature() {
        ...
        this.app.commands.removeCommand(`${this.manifest.id}:"open-workspace-plus-group-modes"`);
    }
    addStatusBarIndicator(modalType = "workspace") {
        ...
        statusBarItem.setAttribute("aria-label", modalType == "mode" ? t("switch-mode") : t("switch-workspace"));
    ...
    onStatusBarClick(evt, modalType) {
        ...
        if (evt.shiftKey === true) {
            ...
            this.registerWorkspaceHotkeys();
            new obsidian.Notice(t(modalType == "mode" ? "saved-mode" : "saved-workspace"));
        }
        else {
    ...
    registerWorkspaceHotkeys() {
        ...
        for (const workspaceName of workspaceNames) {
            this.addCommand({
                id: workspaceName,
                name: t("load-workspace") + workspaceName,
                callback: () => {
                    this.workspacePlugin.loadWorkspace(workspaceName);
                },
            });
